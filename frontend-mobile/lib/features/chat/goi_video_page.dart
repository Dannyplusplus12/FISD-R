...
class _GoiVideoPageState extends ConsumerState<GoiVideoPage> {
  ...
  Future<void> _layIceServers() async {
    try {
      final res = await ApiClient.dio
          .get(ApiEndpoints.turnCredentials, queryParameters: {'nhan_vien_id': widget.phien.id});
      ...
      // Chưa cấu hình TURN — chỉ dùng STUN công cộng, có thể không kết nối được qua NAT nghiêm ngặt.
    }
  }
  ...
  Future<void> _taoKetNoi(int doiPhuongId) async {
    ...
    // Quy tắc tránh xung đột offer: bên có id nhỏ hơn chủ động gửi offer trước.
    if (widget.phien.id < doiPhuongId) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Stack(children: [
          ...
          // Local preview
          Positioned(
            right: 16, top: 16, width: 100, height: 140,
            child: ClipRRect(
          ...
          Positioned(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(width: 16),
              _nutDieuKhien(Icons.call_end, _ketThucCuocGoi, mau: AppColors.danger),
            ]),
          ),
        ]),
      ),
