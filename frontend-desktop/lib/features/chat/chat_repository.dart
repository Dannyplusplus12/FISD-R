...
class ChatRepository {
  ...
  Future<List<KenhChat>> layDanhSachKenh(int nhanVienId) async {
    final res = await _dio.get(ApiEndpoints.kenhChats, queryParameters: {'nhan_vien_id': nhanVienId});
    return (res.data as List).map((e) => KenhChat.fromJson(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<KenhChat> taoKenh({
    ...
    final res = await _dio.post(ApiEndpoints.kenhChats, data: {
      'ten': ten,
  ...
  Future<void> themThanhVien(int kenhId, {required int nhanVienId, required int nguoiThemId}) async {
    await _dio.post(ApiEndpoints.thanhVienKenh(kenhId), data: {
  ...
  Future<void> xoaThanhVien(int kenhId, int nvId, int nguoiXoaId) async {
    await _dio.delete(ApiEndpoints.xoaThanhVienKenh(kenhId, nvId), queryParameters: {'nguoi_xoa_id': nguoiXoaId});
  }

  Future<List<TinNhan>> layTinNhan(int kenhId, {int? beforeId, int limit = 50}) async {
    final res = await _dio.get(ApiEndpoints.tinNhanKenh(kenhId), queryParameters: {
  ...
  Future<TinNhan> guiTinNhan(
    ...
    final res = await _dio.post(ApiEndpoints.tinNhanKenh(kenhId), data: form);
    return TinNhan.fromJson(res.data as Map<String, dynamic>);
  }

  Future<List<NhanVien>> layDanhSachNhanVien() async {
    final res = await _dio.get(ApiEndpoints.nhanViens);
