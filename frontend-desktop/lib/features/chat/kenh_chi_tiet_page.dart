...
class KenhChiTietPage extends ConsumerStatefulWidget {
  ...
  const KenhChiTietPage({super.key, required this.kenh, required this.danhTinh});
...
class _KenhChiTietPageState extends ConsumerState<KenhChiTietPage> {
  ...
  bool get _laChuKenh => widget.kenh.chuKenhId == widget.danhTinh.id;
  ...
  List<ThanhVienChat> get _doiPhuong =>
      widget.kenh.thanhVien.where((t) => t.id != widget.danhTinh.id).toList();
  ...
  void _batDauGoi({required bool coCamera}) {
    ...
    ref.read(realtimeSocketProvider).gui({
      ...
      'data': {
        ...
        'co_camera': coCamera, 'nguoi_goi': widget.danhTinh.ten,
      },
    });
    Navigator.push(context, MaterialPageRoute(builder: (_) => GoiVideoPage(
      ...
      danhTinh: widget.danhTinh, coCamera: coCamera,
    )));
  }
  ...
  void _thamGiaCuocGoi() {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => GoiVideoPage(
      ...
      danhTinh: widget.danhTinh, coCamera: data['co_camera'] == true,
    )));
  }
  ...
  Future<void> _guiText() async {
    ...
    try {
      await ref.read(tinNhanKenhProvider(widget.kenh.id).notifier)
          .gui(nguoiGuiId: widget.danhTinh.id, noiDung: noiDung);
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _guiFiles(List<MultipartFile> files) async {
    ...
    try {
      await ref.read(tinNhanKenhProvider(widget.kenh.id).notifier)
          .gui(nguoiGuiId: widget.danhTinh.id, files: files);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi gửi tệp: $e')));
      }
    } finally {
      if (mounted) setState(() => _dangGui = false);
    }
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          height: 320,
          child: ListView(
            children: nhanVienList.where((nv) => !daCoIds.contains(nv.id)).map((nv) => ListTile(
                  title: Text(nv.ten),
                  ...
                  onTap: () async {
                    ...
                    try {
                      await ref.read(chatRepositoryProvider).themThanhVien(
                            widget.kenh.id,
                            nhanVienId: nv.id,
                            nguoiThemId: widget.danhTinh.id,
                          );
                      ref.invalidate(danhSachKenhProvider(widget.danhTinh.id));
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Đã thêm ${nv.ten} vào kênh')));
                      }
                    } catch (e) {
                      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final mauTieuDe = widget.kenh.laKenhDonHang ? Colors.deepOrange : AppColors.navSelected;
      ...
      body: Column(children: [
        ...
        Expanded(
          child: state.when(
            ...
            data: (list) {
              if (list.isEmpty) {
                return const Center(child: Text('Chưa có tin nhắn nào', style: TextStyle(color: AppColors.textSecondary)));
              }
              return ListView.builder(
                ...
                itemBuilder: (_, i) => _TinNhanBubble(
                  ...
                  laCuaMinh: list[list.length - 1 - i].nguoiGuiId == widget.danhTinh.id,
                ),
              );
            },
          ),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.surface, border: Border(top: BorderSide(color: Colors.grey.shade300))),
          child: Row(children: [
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: laCuaMinh ? AppColors.navSelected : Colors.white,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Padding(
              ...
              child: tep.laAnh
                  ? ClipRRect(
                      ...
                      child: Image.network(tep.url, width: 200, fit: BoxFit.cover,
          ...
          Text(tinNhan.thoiGianGui,
              style: TextStyle(fontSize: 10, color: laCuaMinh ? Colors.white70 : AppColors.textSecondary)),
        ]),
      ),
    );
  }
